...
export class Renderer {
    ...
    async serialize(requestUrl: string, isMobile: boolean):
        ...
        // -------------------------------------------------------------------------------------------------------------
        // Requests to the specified endpoint will be aborted >>> start
        // -------------------------------------------------------------------------------------------------------------
        ...
        try {
            ...
            response = await page.goto(
                requestUrl, {timeout: this.config.timeout, waitUntil: this.config.waitForNavigationUntil});
        } catch (e) {
            console.error(e);
        }
    ...
    async screenshot(
        ...
        try {
            // Navigate to page. Wait until there are no oustanding network requests.
            response =
                await page.goto(url, {timeout: this.config.timeout, waitUntil: this.config.waitForNavigationUntil});
        } catch (e) {
            console.error(e);
        }
